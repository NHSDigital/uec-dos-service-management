...
def test_put_record():
    ...
    assert organisations_record["Item"]["name"] == "Nhs PUT Integrated Care Board"
...
def test_post_record():
    ...
    assert str(response["statusCode"]) == "200"
    organisations_record = table.get_item(Key={"id": mock_id})
    assert organisations_record["Item"]["name"] == "Nhs POST Integrated Care Board"
...
def test_delete_record_by_id():
    ...
    organisations_record = table.get_item(Key={"id": mock_id})
    assert "Item" not in organisations_record
...
def load_sample_event_from_file(filename) -> dict:
    ...
    """
    event_file_name = "./application/organisations-data-manager/test/events/" + filename
