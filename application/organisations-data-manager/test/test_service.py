...
mock_identifier_type = "ODS"
mock_identifier_use = "secondary"
mock_identifier_value = "FF901"
mock_lookup_field = "QYG"
mock_modified_by = "Admin"
mock_name = "Nhs Somewhere And Nowhere Integrated Care Board"
mock_partOf = "3203946975299865"
mock_resourceType = "Organization"
mock_type = "Pharmacy"
...
def create_mock_dynamodb():
    "Create a mock implementation of the dynamodb organisations table"
...
def build_mock_organisations_item():
    ...
    data = {
        ...
        "identifier_type": mock_identifier_type,
        "identifier_use": mock_identifier_use,
        "identifier_value": mock_identifier_value,
        "modifiedBy": mock_modified_by,
        "modifiedDateTime": mock_timestamp_modified,
        "name": mock_name,
        "partOf": mock_partOf,
        "resourceType": mock_resourceType,
        "type": mock_type,
    }
    return data
...
def test_get_record_by_id():
    ...
    assert response["Item"]["identifier_type"] == mock_identifier_type
    assert response["Item"]["identifier_use"] == mock_identifier_use
    assert response["Item"]["identifier_value"] == mock_identifier_value
    ...
    assert response["Item"]["partOf"] == mock_partOf
    assert response["Item"]["resourceType"] == mock_resourceType
    assert response["Item"]["type"] == mock_type
    assert response["Item"]["Address"][0]["city"] == mock_city
    assert response["Item"]["Address"][0]["country"] == mock_country
    assert response["Item"]["Address"][0]["district"] == mock_district
...
def test_add_record():
    ...
    # build record to add
    organisations_record = build_mock_organisations_item()
    organisations_record["active"] = mock_status_update
    organisations_record["id"] = mock_post_id
    service.add_record(organisations_record)
    response = table.get_item(Key={"id": mock_post_id})
...
def test_update_record():
    ...
    # update elements
    organisations_record["active"] = mock_status_update
    organisations_record["name"] = mock_updated_name
    service.update_record(organisations_record)
